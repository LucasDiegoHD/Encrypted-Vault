...
import { X, Sparkles, Edit3, Eye, EyeOff, ShieldCheck, QrCode } from 'lucide-react';
...
export const EditEntryModal: React.FC<EditEntryModalProps> = ({ serviceName, onClose, onSave }) => {
  ...
  useEffect(() => {
    if ((window as any).lockpyAPI?.getCredentials) {
      (window as any).lockpyAPI.getCredentials(serviceName).then((res: any) => {
        if (res && res.status === 'ok' && res.credentials) {
          setUsername(res.credentials.username || '');
          setPassword(res.credentials.password || '');
          setUrl(res.credentials.url || '');
          setTotpSecret(res.credentials.totp_secret || '');
        }
        setFetching(false);
      }).catch(() => setFetching(false));
    } else {
      setUsername('user@account');
      setPassword('SuperSecretPass123!');
      setFetching(false);
    }
  }, [serviceName]);
      ...
      <div className="w-full max-w-lg glass-card rounded-3xl border border-slate-700/60 shadow-2xl overflow-hidden p-6 space-y-5">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Edit3 className="w-5 h-5 text-sky-400" /> Edit Credential
          </h3>
          <button
            onClick={onClose}
        ...
        {fetching ? (
          <div className="py-8 text-center text-xs text-slate-400">Loading credential details...</div>
        ) : (
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-3.5 pr-10 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:border-sky-500 font-mono"
                  required
                />
                <button
                  type="button"
